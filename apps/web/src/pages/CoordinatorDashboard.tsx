...
import { isHeadCoordinator } from '../lib/roles';
...
const CoordinatorDashboard: React.FC = () => {
  ...
  const leadsBasePath = useMemo(() => (isHeadCoordinator(user) ? '/center/leads' : '/coordinator/leads'), [user]);

  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const baseRequests = [apiClient.get('/api/dashboard/coordinator/overview'), fetchCenterDashboard()];
